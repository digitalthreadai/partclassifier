...
def process_part(
    ...
    for source in (api_sources or []):
        try:
            ...
            if result and result.attributes and len(result.attributes) >= 3:
                attributes, lov_mismatches, _ = normalize_attrs_with_lov_status(result.attributes, part_class)
    ...
    # NOTE: Class-blind validation (as in main.py) is not applied here because
    # main_cc.py uses Claude CLI (not async LLM client). The class-aware extraction
    # and existing reclassification logic below handle most cases.
